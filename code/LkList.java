...
public class LkList {
     ...
     * 第1种方法:用栈
     * 第2种方法:快慢指针
     * @Parm first1 : 第一个节点(非头节点)
     */
    public Node getComNodeByStack(Node first1, Node first2) {
        Node curr1 = first1;
        Node curr2 = first2;
        ...
        while (!stack1.isEmpty() && !stack2.isEmpty()) {
            ...
            Node node11 = stack1.peek();
            Node node2 = stack2.pop();
            Node node22 = stack2.peek();

            if (!node1.data.equals(node2.data)) {
                return null;
            } else if (!node11.data.equals(node22.data)) {
                return node1;
            }
        }
     ...
     * @Parm first1 : 第一个节点(非头节点)
     */
    public Node getComNodeByQuickSlow(Node head1, Node head2) {
        Node curr1 = head1;
        Node curr2 = head2;
        int size1 = 0;
        int size2 = 0;
        ...
        curr2 =head2;
        //元素个数差
        //关键:长度较长的链表向后移动diff, 与短链表对齐,
        //此时再同步后移, 则第一个相同的元素即为交点
        int diff = Math.abs(size1-size2);
        int minSize = 0;
        //head1元素个数大于head2,size1==size2时直接从第1个元素开始比较即可
        if (size1>size2) {
        ...
        //head1元素个数小于head2,size1==size2时直接从第1个元素开始比较即可
        if (size1<size2) {
            for (int i=0;i<diff;i++) {
                curr2 = curr2.next;
            }
    ...
    public static void main(String[] args) {
        ...
        lkList.addLast("5");
        lkList.addLast("7");
        LkList lkList1 = new LkList();
        lkList1.addLast("2");
        lkList1.addLast("4");
        lkList1.addLast("6");
        lkList1.addLast("8");
        lkList.getComNodeByStack(lkList.head.next, lkList1.head.next);
        System.out.println("");
    }
